...
- 105/105 tests passed on hpubuntu; Windows: 104 passed, one Linux-only test skipped.
...
Signed-in external browser playback must be checked through the owner's normal MFA
session; direct private-network playback does not substitute for that access-path check.
No credentials or cookies should be copied into a qualification script to avoid MFA.
