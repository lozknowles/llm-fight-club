...
export function voiceLabRoutes({ directory, key, enabled, provider, judge }) {
  const lab = new VoiceLab({ directory, provider: provider || new OmniVoiceEnrolmentProvider({
  ...
  return async (req, res, url) => {
    ...
    if (incoming.length !== expected.length || !timingSafeEqual(incoming, expected)) { reply(res, 401, { error: 'Voice Lab authorisation required' }); return true; }
    ...
    try {
      ...
      if (route === '/config' && req.method === 'GET') result = { consent_version: CONSENT_VERSION, prompts: PROMPTS, test_phrases: TEST_PHRASES, roles: ROLES };
        ...
        else if (action === 'samples') result = await lab.sample(id, input.index, Buffer.from(input.audio || '', 'base64'), input.confirmed_text);
        else if (action === 'build') result = await lab.build(id);
        else if (action === 'test') result = await lab.test(id);
