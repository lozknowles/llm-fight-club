...
export class PublicAccess {
  constructor({ enabled = false, origins = [], cookiePath = '/llm-debate/', secure = true,
    clock = Date.now, models = [] } = {}) {
    Object.assign(this, { enabled, origins, cookiePath, secure, clock, models });
  ...
  creation(input, visitor) {
    ...
    for (const p of input.participants) {
      if (!this.models.includes(p.model) || !PUBLIC_VOICES.some(v => v.id === p.voice)) fail(400, 'Choose a model and voice offered on this page.');
      p.voiceProfile = null;
    ...
    return { ...input, turnLimit: Math.min(6, Math.max(2, Number(input.turnLimit) || 4)), turnLength: 30,
      interruptionLevel: 'OFF', speechProfile: 'LOCAL', speechMode: 'server' };
  }
  generation(visitor) {
  ...
  speech(payload, visitor, conversations) {
    if (!payload || !PUBLIC_VOICES.some(v => v.id === payload.voice) || String(payload.text || '').length > 4000) fail(400, 'Invalid speech request');
