...
export function completePlayback(conversation, payload = {}) {
  ...
  if (payload.ttsMetrics && typeof payload.ttsMetrics === 'object') row.tts_metrics = structuredClone(payload.ttsMetrics);
  ...
  if (telemetry) {
    ...
    if (row.tts_metrics) telemetry.tts_metrics = structuredClone(row.tts_metrics);
  }
  return result;
}
