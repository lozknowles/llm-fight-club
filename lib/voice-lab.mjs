...
export class VoiceLab {
  ...
  async get(id) { return JSON.parse(await fs.readFile(path.join(this.folder(id), 'profile.json'), 'utf8')); }
  async list() {
  ...
  async create(input) {
    ...
    return this.exclusive('new-profile', async () => {
      ...
      return this.write({ profile_id: randomUUID(), display_name: name, voice_provider: this.provider.id,
        ...
        consent_confirmations: { permission: true, synthetic: true, purpose: true }, samples: [], tests: [],
        qualification_status: 'RECORDING', technical_generation_success: false, user_acceptance: false,
        approved_roles: [], revision: 0, disclosure: DISCLOSURE, publication_disclosure: true });
    });
  }
  async sample(id, index, bytes, confirmedText) {
    return this.exclusive(id, async () => {
      ...
      requireThat(Number.isInteger(index) && index >= 0 && index < 12 && confirmedText === true, 'Confirm that the recording matches its guided text');
      ...
      p.samples.push({ index, text: PROMPTS[index % 6].text, hash: hash(bytes), quality, accepted_at: stamp() });
  ...
  async build(id) {
    return this.exclusive(id, async () => {
      const p = await this.get(id);
      requireThat(p.qualification_status === 'RECORDING' && PROMPTS.every(prompt => p.samples.some(s => s.index === prompt.index)), 'Accept all six guided samples first');
      ...
      Object.assign(p, { voice_representation_hash: hash(result.representation), reference_audio_hashes: p.samples.map(s => s.hash),
        ...
        qualification_status: 'UNQUALIFIED', tests: [], technical_generation_success: false, user_acceptance: false, approved_roles: [], revision: p.revision + 1 });
      return this.write(p);
    });
  }
  ...
  async test(id) {
    return this.exclusive(id, async () => {
      const p = await this.get(id);
      requireThat(['UNQUALIFIED', 'TESTED'].includes(p.qualification_status), 'Create or re-qualify the profile first');
      requireThat(p.tests.length < 12, 'Test limit reached; re-qualify to start a new comparison');
      const text = TEST_PHRASES[p.tests.length % TEST_PHRASES.length];
      ...
      p.tests.push(test); p.technical_generation_success = true; p.qualification_status = 'TESTED';
      return this.write(p);
    });
  }
  async heard(id, testId, kind) {
    return this.exclusive(id, async () => {
      ...
      requireThat(p.qualification_status === 'TESTED' && t && ['original', 'synthetic'].includes(kind), 'No current comparison');
  ...
  async reset(id, action) {
    return this.exclusive(id, async () => {
      ...
      p.user_acceptance = false; p.approved_roles = []; p.technical_generation_success = false; p.revision++;
