...
export function sendBattleAudio(req, res, bytes) {
  let start = 0, end = bytes.length - 1, status = 200;
  if (req.headers.range) {
    const match = String(req.headers.range).match(/^bytes=(\d*)-(\d*)$/);
    if (!match || (!match[1] && !match[2])) { res.writeHead(416, { 'content-range': `bytes */${bytes.length}` }); res.end(); return; }
  ...
  res.writeHead(status, { 'content-type': 'audio/wav', 'content-length': end - start + 1, 'accept-ranges': 'bytes',
    'cache-control': 'private, no-store', 'x-content-type-options': 'nosniff' });
  res.end(bytes.subarray(start, end + 1));
}

export async function preparedBattleRoutes({ directory, models, modelIds, enabled, speechUrl, speechToken, fastUrl, replayOnly = false }) {
  ...
  const fastSpeech = { capabilities: () => fastStore.capabilities(), synthesise: async (input, signal) => {
  ...
  return async (req, res, url) => {
    ...
    try {
      ...
      if (replayOnly && req.method === 'POST' && !url.pathname.endsWith('/playback')) throw Error('Read-only evidence replay: generation is disabled');
      if (url.pathname === '/api/prepared-battles/config' && req.method === 'GET') {
        ...
        reply(res, 200, { models: modelIds, speechHealth: health, replayOnly, captions: 'approximate word progress, not forced alignment' });
