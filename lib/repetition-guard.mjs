...
const INTERVIEW_LENSES = [
  'the clearest concrete evidence or example',
  'where the belief originated and what motive sustains it',
  'how it would work in practice',
  'who benefits from it',
  'who pays or carries the risk',
  'the likeliest failure mode',
  'a contradiction with an earlier commitment',
  'the character’s personal stake or emotional investment',
  'how another participant or institution would react',
  'the public or cultural consequence',
  'the ethical consequence',
  'a credible alternative the character rejects',
  'what evidence could change the character’s mind',
  'an unexpected second-order consequence',
  'the difficult implementation detail',
  'who should be accountable when it fails',
  'what happens five years later',
  'one point the character might concede',
  'the strongest remaining inconsistency',
  'a concise closing proposition',
];

export function interviewProgressionLens(turnIndex) {
  const contribution = Math.max(1, Math.floor(Number(turnIndex || 0) / 2));
  return INTERVIEW_LENSES[Math.min(contribution - 1, INTERVIEW_LENSES.length - 1)];
}
