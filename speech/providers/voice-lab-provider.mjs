...
export class OmniVoiceEnrolmentProvider {
  ...
  async call(route, payload) {
    ...
    try { response = await fetch(`${this.baseUrl.replace(/\/$/, '')}/voice-lab/${route}`, {
      ...
      body: payload ? JSON.stringify(payload) : undefined, signal: AbortSignal.timeout(180000),
  ...
  async synthesize({ representation, version, text }) {
    const data = await this.call('synthesize', { representation: representation.toString('base64'), version, text });
